...
x_train_fusion = np.asarray(data["xtrain_fusion"])
x_test_fusion = np.asarray(data["xtest_fusion"])
y_train = np.asarray(data["ytrain_fusion"])
y_test = np.asarray(data["ytest_fusion"])

input_data = tf.keras.Input(shape=x_train_fusion.shape[1:])
input_data_lidar = input_data[:,:,:,0:59,:]

x = tf.keras.layers.Conv3D(12, (3,3,3))(input_data_lidar)
...
#dropout = tf.keras.layers.Dropout(0.55)(dense)
output_layer_lidar = tf.keras.activations.relu(flatten)

###############################################################################

input_data_radar = tf.squeeze(input_data[:,:,:,59:60,:],axis=4)

x = tf.keras.layers.Conv2D(12, (3,3))(input_data_radar)
x = tf.keras.layers.BatchNormalization()(x)
x = tf.keras.activations.relu(x)
...
output_layer_radar = tf.keras.activations.relu(flatten)

######################################################################################

fusion = tf.concat([output_layer_lidar,output_layer_radar],1)
dense = tf.keras.layers.Dense(64)(fusion)
relu_dense = tf.keras.activations.relu(dense)
dense2 = tf.keras.layers.Dense(12)(relu_dense)
...
history=model.fit(x=x_train_fusion,
        y=y_train,
        batch_size=BATCH,
        epochs=EPOCH,
        validation_data=(x_test_fusion, y_test))

test_loss, test_acc = model.evaluate(x_test_fusion,  y_test, verbose=2)
...
plt.savefig('C:/Users/sb3682.ECE-2V7QHQ3/My Stuff/Traffic_Sign/Radar+Camera/results/featurefusion_confusion_matrix.png')
plt.show()
...
# plt.legend()
# plt.show()
# plt.savefig('C:/Users/sb3682.ECE-2V7QHQ3/My Stuff/Traffic_Sign/Radar+Camera/results/Lidar_train_vs_val.png')
