...
data = h5py.File("C:/Users/sb3682.ECE-2V7QHQ3/My Stuff/Traffic_Sign/Radar+Camera/Dataset/radar_data_shuffle.h5", 'r')
x = np.asarray(data["x_radar"])
y = np.asarray(data["y_radar"])

# x_train = np.asarray(data["xtrain_radar"])
# y_train = np.asarray(data["ytrain_radar"])
# x_test = np.asarray(data["xtest_radar"])
# y_test = np.asarray(data["ytest_radar"])
...
x = tf.keras.layers.Conv2D(12, (3,3))(input_data)
...
plt.savefig('C:/Users/sb3682.ECE-2V7QHQ3/My Stuff/Traffic_Sign/Radar+Camera/results/Radar_confusion_matrix.png')
plt.show()
